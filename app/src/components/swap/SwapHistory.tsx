...
export function SwapHistory() {
    ...
    return (
      <div className="card">
        <div className="flex items-center justify-center py-12">
          <svg className="text-primary-500 h-8 w-8 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle
    ...
    return (
      <div className="card">
        <div className="py-12 text-center">
          <div className="text-red-500 dark:text-red-400">Failed to load swap history</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="card">
      <h2 className="text-surface-900 mb-4 text-lg font-semibold dark:text-white">Swap History</h2>
      <div className="space-y-4">
        {receipts.map((receipt: SwapReceipt) => (
          <div
            key={receipt.id}
            className="border-surface-200 dark:border-surface-700 overflow-hidden rounded-lg border"
          >
            {/* Main row */}
            <div
              className="hover:bg-surface-50 dark:hover:bg-surface-800 flex cursor-pointer items-center justify-between p-4 transition-colors"
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <svg
                  className={`text-surface-400 h-5 w-5 transition-transform ${
            ...
            {expandedReceipt === receipt.id && receipt.status === 'confirmed' && (
              <div className="border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 border-t p-4">
                <SwapComparison
                  receiptId={receipt.id}
                  inputAmount={String(receipt.inputAmount)}
                  inputMint={receipt.inputMint}
